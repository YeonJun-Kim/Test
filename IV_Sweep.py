...
class IVSweep(ttk.Frame):
    ...
    def __init__(self, master, main_window = None):
        ...
        self.sourcetype = "Current source"
        ...
        self.df_config = pd.read_csv('Auto_Config_CSource.csv')
        ...
        # print('cinmin_list : ', self.cinmin_list )
        # print('cinpl_list : ', self.cinpl_list )
    ...
    def create_widgets(self):
        ...
        # Parameter Header
        self.header_parameter = ttk.Label(parameter_frame, text="Current Source Parameter", font=header_font)
        ...
        # Start Voltage Entry
        self.label_start_current = ttk.Label(parameter_frame, text="Start Current (mA):")
        self.label_start_current.grid(row=2, column=0, padx=10, pady=10)
        self.entry_start_current = ttk.Entry(parameter_frame)
        self.entry_start_current.insert(0, "-1")
        self.entry_start_current.grid(row=2, column=1, padx=10, pady=10)

        # Stop Voltage Entry
        self.label_stop_current = ttk.Label(parameter_frame, text="Stop Current (mA):")
        self.label_stop_current.grid(row=3, column=0, padx=10, pady=10)
        self.entry_stop_current = ttk.Entry(parameter_frame)
        self.entry_stop_current.insert(0, "3")
        self.entry_stop_current.grid(row=3, column=1, padx=10, pady=10)

        # Step Voltage Entry
        self.label_step_current = ttk.Label(parameter_frame, text="Step (mA):")
        self.label_step_current.grid(row=4, column=0, padx=10, pady=10)
        self.entry_step_current = ttk.Entry(parameter_frame)
        self.entry_step_current.insert(0, "0.1")
        self.entry_step_current.grid(row=4, column=1, padx=10, pady=10)

        # Forward Compliance Current entry
        self.label_compliacncef = ttk.Label(parameter_frame, text="Forward Limit (V):")
        ...
        self.label_compliacncer = ttk.Label(parameter_frame, text="Reverse Limit (V):")
        ...
        # Measure Range entry
        self.label_range = ttk.Label(parameter_frame, text="Measure range (V):")
    ...
    def toggle_AutoSet(self):
        ...
        if self.CheckVar2.get():
            ...
            self.combo_probing.grid(row=3, column=1, padx=10, pady=10)
            # self.entry_start_current.config(state= "disabled")
            # self.entry_stop_current.config(state= "disabled")

        else :
            self.combo_probing.grid_remove()            
            self.entry_probing.grid(row=3, column=1, padx=10, pady=10)
            self.entry_start_current.config(state= "enabled")
            self.entry_stop_current.config(state= "enabled")
    ...
    def probe_IO(self,event):
        ...
        cinmin = self.cinmin_list[tmp_index]
        cinpl = self.cinpl_list[tmp_index]
        vcomp = self.vcomp_list[tmp_index]
        ...
        self.entry_start_current.insert(0,cinmin)
        self.entry_stop_current.insert(0,cinpl)
        self.entry_compliacncef.insert(0,vcomp)
    ...
    def IVplot(self):
        ...
        self.ax.set_title('IV Curve (mA-V)')
    ...
    def start_measurement(self):
        ...
        autorange_selected = self.combo_autorange.get()
        selected_type = self.combo_sweep_type.get()
        start_current = float(self.entry_start_current.get())
        stop_current = float(self.entry_stop_current.get())
        step_current = float(self.entry_step_current.get())
        ...
        start_current = start_current / 1000
        stop_current = stop_current / 1000
        step_current = step_current / 1000
        # comp_voltagef = comp_voltagef / 1000
        # comp_voltager = comp_voltager / 1000
        ...
        if not os.path.exists(result_path):
            ...
            df = pd.DataFrame({"Label": [1] * N_measure, "Sourcetype" : [self.sourcetype]* N_measure, "SerialNumber": [module_SN] * N_measure, "Measurement_type": [measurementType] * N_measure, "Channel": [selected_channel] * N_measure,
                               "Sense_mode": [selected_sensemode] * N_measure, "Sweep_Type": [selected_type] * N_measure, "Forward_Limit(V)": [float(comp_voltagef)] * N_measure,
                               "Reverse_Limit(V)": [float(comp_voltager)] * N_measure, "Hold_On_Delay(ms)": [on_delay] * N_measure, "Pulse_Off_Time(ms)": [off_delay] * N_measure,
                               "Autorange": [autorange_selected] * N_measure, "Measure_Range(V)": [mrange] * N_measure, "Autozero": [autozero] * N_measure,
                               "Pin": [probe_point] * N_measure, "Voltage[V]": voltages, "Current[A]": current_abs_A, "Current[mA]": current_abs_mA, "NPLC": [nplc] * N_measure})
            df_result = df.copy()
            df_result.to_csv(result_path, index=False)
            ...
            df2 = pd.DataFrame({"Label": [max_label + 1] * N_measure, "Sourcetype" : [self.sourcetype]* N_measure, "SerialNumber": [module_SN] * N_measure, "Measurement_type": [measurementType] * N_measure, "Channel": [selected_channel] * N_measure,
                               "Sense_mode": [selected_sensemode] * N_measure, "Sweep_Type": [selected_type] * N_measure, "Forward_Limit(V)": [float(comp_voltagef)] * N_measure,
                               "Reverse_Limit(V)": [float(comp_voltager)] * N_measure, "Hold_On_Delay(ms)": [on_delay] * N_measure, "Pulse_Off_Time(ms)": [off_delay] * N_measure,
                               "Autorange": [autorange_selected] * N_measure, "Measure_Range(V)": [mrange] * N_measure, "Autozero": [autozero] * N_measure,
                               "Pin": [probe_point] * N_measure, "Voltage[V]": voltages, "Current[A]": current_abs_A, "Current[mA]": current_abs_mA, "NPLC": [nplc] * N_measure})
    ...
    def setup_measurement(self, keithley, channel="Channel A", sensemode=0, nplc=1, autozero="AUTOZERO_AUTO", autorange=1, mrange="nan"):
        if channel == "Channel A":
            ...
            keithley.write("smua.source.autorangei = 1")
            if str(autorange) == "1":
                keithley.write("smua.measure.autorangev = 1")
            elif str(autorange) == "2":
                keithley.write("smua.measure.autorangev = 2")
            elif str(autorange) == "0":
                keithley.write("smua.measure.autorangev = 0")
                keithley.write("smua.measure.rangev = " + str(mrange))
            ...
            keithley.write("smub.source.autorangei = 1")
            if str(autorange) == "1":
                keithley.write("smub.measure.autorangev = 1")
            elif str(autorange) == "2":
                keithley.write("smub.measure.autorangev = 2")
            elif str(autorange) == "0":
                keithley.write("smub.measure.autorangev = 0")
                keithley.write("smub.measure.rangev = " + str(mrange))
            ...
            keithley.write("display.smub.measure.func = display.MEASURE_DCVOLTS")
    ...
    def measure_iv_curve_staircase(self, keithley, channel, start_current, stop_current, step_current, comp_voltagef, comp_voltager, on_delay):
        ...
        for current in np.arange(start_current, stop_current + step_current, step_current):
            ...
            if channel == "Channel A":
                if current < 0:
                    keithley.write("smua.source.limitv = " + str(comp_voltager))
                else:
                    keithley.write("smua.source.limitv = " + str(comp_voltagef))
                keithley.write("smua.source.leveli = " + str(current))
                keithley.write("smua.source.output = smua.OUTPUT_ON")
                time.sleep(float(on_delay) / 1000)
                keithley.write("smua.measure.v(smua.nvbuffer1)")
                voltage = keithley.query("printbuffer(1,1,smua.nvbuffer1.readings)")
                ...
                if current < 0 :
                    keithley.write("smub.source.limitv = " + str(comp_voltager))

                else :
                    keithley.write("smub.source.limitv = " + str(comp_voltagef))

                keithley.write("smub.source.leveli = " + str(current))
                keithley.write("smub.source.output = smub.OUTPUT_ON")
                time.sleep(float(on_delay) / 1000)
                keithley.write("smub.measure.v(smub.nvbuffer1)")
                voltage = keithley.query("printbuffer(1,1,smub.nvbuffer1.readings)")
                print("Voltage measured : ", voltage)
                currents.append(current)
                voltages.append(voltage)
...
if __name__ == "__main__":
    ...
    tab1 = IVSweep(notebook)
    notebook.add(tab1, text='IV Sweep (Current Source)')    
    # app = IVSweep(root)
    root.mainloop()