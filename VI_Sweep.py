...
class VISweep(BaseSweep):
    def __init__(self, master, main_window=None):
        config = {
            "source_type": "Voltage source",
            "config_file": "Auto_Config_VSource.csv",
            "param_header": "Voltage Source Parameter",
            "start_label": "Start Voltage (V):",
            "start_default": "-3",
            "stop_label": "Stop Voltage (V):",
            "stop_default": "3",
            "step_label": "Step (V):",
            "fwd_limit_label": "Forward Limit (mA):",
            "rev_limit_label": "Reverse Limit (mA):",
            "measure_range_label": "Measure range (mA):",
            "plot_title": "IV Curve (V-mA)",
            "plot_xlabel": "Voltage (V)",
            "plot_ylabel": "Current (mA)",
        }
    ...
    def start_measurement(self):
        ...
        start_voltage = float(self.entry_start.get())
        stop_voltage = float(self.entry_stop.get())
        step_voltage = float(self.entry_step.get())
        ...
        df_new_data = pd.DataFrame({
            ...
            "Forward_Limit(mA)": [1000 * float(comp_currentf)] * N_measure,
            "Reverse_Limit(mA)": [1000 * float(comp_currentr)] * N_measure, 
            "Hold_On_Delay(ms)": [on_delay] * N_measure, 
            "Pulse_Off_Time(ms)": [off_delay] * N_measure,
            "Autorange": [autorange_selected] * N_measure, 
            "Measure_Range(mA)": [mrange * 1000 if mrange != 'nan' else 'nan'] * N_measure, 
            "Autozero": [autozero] * N_measure,
            "Pin": [probe_point] * N_measure, 
            "Voltage[V]": voltages, 
            "Current[A]": current_abs, 
            "Current[mA]": current_abs_mA, 
            "NPLC": [nplc] * N_measure
        })
        ...
        self.plot()
        print("VI measurement finished and saved successfully")
        print("##############################################################")
        self.button_abort['state'] = 'disabled'
        keithley.close()
    ...
    def setup_measurement(self, keithley, channel, sensemode, nplc, autozero, autorange, mrange):
        ...
        keithley.write(f"{smu}.source.func = {smu}.OUTPUT_DCVOLTS")
        keithley.write(f"{smu}.source.autorangev = 1")
        
        if str(autorange) == "0":
            keithley.write(f"{smu}.measure.autorangei = 0")
            keithley.write(f"{smu}.measure.rangei = {mrange}")
        else:
            keithley.write(f"{smu}.measure.autorangei = {autorange}")
        ...
        keithley.write(f"display.{smu}.measure.func = display.MEASURE_DCAMPS")
    ...
    def measure_iv_curve_staircase(self, keithley, channel, start_voltage, stop_voltage, step_voltage, comp_currentf, comp_currentr, on_delay):
        ...
        for voltage in np.arange(start_voltage, stop_voltage + step_voltage, step_voltage):
            ...
            limit = comp_currentr if voltage < 0 else comp_currentf
            keithley.write(f"{smu}.source.limiti = {limit}")
            keithley.write(f"{smu}.source.levelv = {voltage}")
            keithley.write(f"{smu}.source.output = {smu}.OUTPUT_ON")
            time.sleep(float(on_delay) / 1000)
            keithley.write(f"{smu}.measure.i({smu}.nvbuffer1)")
            current = keithley.query(f"printbuffer(1,1,{smu}.nvbuffer1.readings)")
